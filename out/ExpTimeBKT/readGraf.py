...
# Citesc toate liniile si le salvez in lines
...
# Aloc toate nodurile
for node in range(1,nrNodes + 1):
    nodes.append(Nod(node))

# Aloc un graf cu id 0, numarul de noduri nrNodes si nodurile nodes
graf = Graf(0,nrNodes,nodes)

# Salvez toate legaturile
for line in lines[3:]:
    linkNodes = line.split(" ")
    first = int(linkNodes[0])
    second = int(linkNodes[1])
...
res = False
res = solveKClique.kClique(graf.nodes,k,0,[],res)
print(res)
